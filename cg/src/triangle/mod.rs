...
pub async fn draw_triangle(event_loop: EventLoop<()>, window: Window) {
    ...
    let pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
        label: Some("pipeline-layout-for-triangle"),
        bind_group_layouts: &[],
    ...
    let render_pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
        label: Some("render-pipeline-for-triangle"),
        ...
        *control_flow = ControlFlow::Wait;
        match event {
            ...
            Event::RedrawRequested(_) => {
                ...
                let mut encoder =
                    device.create_command_encoder(&wgpu::CommandEncoderDescriptor { label: Some("command-encoder-for-triangle") });
                {
