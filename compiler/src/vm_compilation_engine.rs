...
use crate::{compilation_engine::CompilationEngine, tokens::Token, sym_table::{SymbolTable, VarType}, code_generator::{Segment, CodeGenerator, Command}};
...
impl CompilationEngine for VmCompilationEngine {
    ...
    fn compile_if(&mut self) {
        ...
        self.code_gen.write_if(&format!("{}.IF_FALSE${}", self.class_name,self.if_count));
        
        self.pop_symbol_assert("{");
        self.compile_statements();
        self.pop_symbol_assert("}");

        self.code_gen.write_goto(&format!("{}.IF_END${}", self.class_name,self.if_count));
        self.code_gen.write_label(&format!("{}.IF_FALSE${}", self.class_name,self.if_count));
        ...
        self.code_gen.write_label(&format!("{}.IF_END${}", self.class_name,self.if_count));
        self.if_count += 1;
    }
    ...
    fn compile_while(&mut self) {
        ...
        self.code_gen.write_label(&format!("{}.WHILE_START${}", self.class_name,self.while_count));
        self.pop_symbol_assert("(");
        self.compile_expression();
        self.pop_symbol_assert(")");

        self.code_gen.write_arithmetic(&Command::Not);
        self.code_gen.write_if(&format!("{}.WHILE_END${}", self.class_name,self.while_count));

        self.pop_symbol_assert("{");
        self.compile_statements();
        self.pop_symbol_assert("}");
        self.code_gen.write_goto(&format!("{}.WHILE_START${}", self.class_name,self.while_count));
        
        self.code_gen.write_label(&format!("{}.WHILE_END${}", self.class_name,self.while_count));
        self.while_count += 1;
    }
    ...
    fn compile_do(&mut self) {
        ...
        match self.tokens.peek() {
            ...
            Some(Token::Symbol(t)) if t == "." => {
                ...
                self.pop_symbol_assert(")");
                if self.sym_table.has_id(&obj_or_class_name) {
                    self.code_gen.write_call(&format!("{}.{}",self.sym_table.type_of(&obj_or_class_name),method_name), n_args+1);
                }else{
                    self.code_gen.write_call(&format!("{}.{}",obj_or_class_name,method_name), n_args);
                }
            }
            _ => {
                panic!()
                ...
                match self.tokens.peek() {
                    ...
                    Some(Token::Symbol(t)) if t == "." => {
                        ...
                        if self.sym_table.has_id(&obj_or_class_name) {
                            let kind = self.sym_table.kind_of(&obj_or_class_name);
                            let seg = kind.into();
                            self.code_gen.write_push(&seg, self.sym_table.index_of(&obj_or_class_name));
                        }
                        self.pop_symbol_assert(".");
                        let method_name = self.pop_identifier();
                        self.pop_symbol_assert("(");
