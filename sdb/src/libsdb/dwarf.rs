...
impl Die {
    pub fn get_bitfield_information(&self, class_byte_size: u64) -> Result<Option<BitfieldInformation>, SdbError> {
        if !self.contains(DW_AT_bit_offset.0 as u64) && !self.contains(DW_AT_data_bit_offset.0 as u64) {
            return Ok(None);
        }
